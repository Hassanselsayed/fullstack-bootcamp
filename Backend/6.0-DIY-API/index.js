...
app.get("/filter", async (req, res) => {
  const filteredJokes = jokes.filter(j => j.jokeType === req.query.type);
...
app.put("/jokes/:id", async (req, res) => {
  ...
  const oldJokeIndex = jokes.findIndex(j => j.id === jokeId);
  ...
  if (oldJokeIndex !== -1) {
    ...
    res.send(`Couldn't find a joke with an id ${jokeId}.`)
  }
});
...
app.patch("/jokes/:id", async (req, res) => {
  ...
  const oldJoke = jokes.find(j => j.id === jokeId);
  const oldJokeIndex = jokes.indexOf(oldJoke);
...
app.delete("/jokes/:id", async (req, res) => {
  ...
  if (jokeIndex !== -1) {
    ...
    res.json(`Joke with id ${jokeId} deleted.`);
  } else {
    res.send(`Couldn't find a joke with an id ${jokeId}.`);
  }
});
...
app.delete("/all", async (req, res) => {
  if (req.query.key === masterKey) {
    ...
    res.send("Not authorized to make that request.");
  }
});
