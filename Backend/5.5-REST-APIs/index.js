...
app.post("/post-secret", async (req, res) => {
  ...
  try {
    if (secret && score) {
      ...
      res.render("index.ejs", { content: "Please add both secret and score to post a new entry." });
    }
  } catch (error) {
    res.render("index.ejs", { content: JSON.stringify(error.response.data) });
  }
...
app.post("/put-secret", async (req, res) => {
  ...
  try {
    if (body.secret && body.score) {
      const result = await axios.put(API_URL + "/secrets/" + searchId, body, config);
      res.render("index.ejs", { content: JSON.stringify(result.data) });
    } else {
      res.render("index.ejs", { content: "Please add both secret and score to put a new entry." });
    }
  } catch (error) {
    res.render("index.ejs", { content: JSON.stringify(error.response.data) });
  }
